...
5. `POST /api/v1/goal/contributions { signature }` until it stops answering 202; the server verifies the transaction on chain and records it immediately (the cron would find it within a minute anyway). The confirmation carries the round's figures (`raised_usd`, `target_usd`, `contributions`, `contributors`) and `excluded` (an operator wallet: counted on the meter, never in the draw); the sheet turns into a success state that says which of the three applies, and redraws the card's meter and counts in place. While a page with the card is open it also refreshes the figures from `GET /api/v1/goal` every 30 seconds and when the tab becomes visible, and reloads when the round's status changes.
