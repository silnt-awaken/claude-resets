...
**Community goal + RESET token** (Robinhood Chain): readers pool USDC, one free entrant wins a month of Claude Max 20x by a verifiable block-hash draw; RESET supply burns on every published reset. Contract in `contracts/`, mechanics and launch steps in [docs/goal-and-token.md](docs/goal-and-token.md).
