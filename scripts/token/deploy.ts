// npm run token:deploy -- --pool <0xPoolWallet> [--rpc <url>] [--dry-run]
...
const gas = await publicClient.estimateContractGas({ abi: artifact.abi as never, bytecode: artifact.bytecode, args: [pool], account: account.address } as never).catch(() => null);
...
const hash = await wallet.deployContract({ abi: artifact.abi as never, bytecode: artifact.bytecode, args: [pool] } as never);
...
console.log('Next: add liquidity on Uniswap, mark the LP pair and treasury fee-exempt (setFeeExempt), then renounceOwnership when you are done configuring.');
